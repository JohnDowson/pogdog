...
def resolve_tag(guild, txt):
    ...
    def repl_tag(match: re.match) -> str:
        ...
        return match[0]
    return re_discord_tag.sub(repl_tag, txt)


def cloud(messages, guild):
    g = defaultdict(lambda: 0)
    filter = ["в", "и", "с", "не", "на", "ты",
              "я", "как", "что", "а", "бы", "из"]
    for message in messages:
        for token in tokenize(message):
            if token not in filter:
                g[resolve_tag(guild, token)] += 1
    res = {}
    count = len(g)
    for word, freq in zip(g.keys(), g.values()):
        res[word] = freq/count
    return res
